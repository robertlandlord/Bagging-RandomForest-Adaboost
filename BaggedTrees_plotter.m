function [oob_err, test_err] = BaggedTrees_plotter(X_tr, y_tr, X_te, y_te, numBags)
...
%     if size(find(sizes==0))==0
%         agg_preds = round(sums./sizes);
%         oob_err(i) = sum(abs(y_tr-agg_preds))/n;
%     end
    nonzero_indices = find(sizes~=0);
    nonzero_sizes = sizes(nonzero_indices);
    nonzero_sums = sums(nonzero_indices);
    
    
    agg_preds = round(nonzero_sums ./ nonzero_sizes);
    y_tr_nonzero = y_tr(nonzero_indices);
    oob_err(i) = sum(abs(y_tr_nonzero-agg_preds))/size(nonzero_indices,1);
end
